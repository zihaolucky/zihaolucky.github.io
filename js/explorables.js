...
(function () {
  ...
  function initNluRouter(root) {
    ...
    var rowRefs = NLU_CASES.map(function (item) {
      ...
      var ruleLine = item.rule
        ? 'Template: ' + item.rule.note + ' → <b>' + item.rule.domain + '</b>, match ' + item.rule.score.toFixed(2)
        : 'Template: <b>nothing matches</b>, so the model runs';
      src.innerHTML = ruleLine + '<br>Model, if it runs: <b>' + item.model.domain + '</b>';
      ...
      return { item: item, dialMark: dialMark, dot: dot, cap: cap, verdict: verdict, win: win };
    });
